...
PixelDrawer.prototype.drawCircleWidthDirtyHack = function (_xc, _yc, _a, _b, _color, _width)
{
  ...
  var dxt = 2*b2*x, dyt = -2*a2*y;
  var d2xt = 2*b2, d2yt = 2*a2;
  var width = Math.floor(_width/2);
    
  while (y>=0 && x<=_a) {
        
    this.fillCircle(_xc+x, _yc+y, _xc+x+width, _yc+y+width, _color);
    if (x!=0 || y!=0)
      this.fillCircle(_xc-x, _yc-y,_xc-x+width, _yc-y+width, color);
    if (x!=0 && y!=0) {
      this.fillCircle(_xc+x, _yc-y,_xc+x+width, _yc-y+width, color);
      this.fillCircle(_xc-x, _yc+y,_xc-x+width, _yc+y+width, color);
    }
...
PixelDrawer.prototype.floodFill = function (_x, _y, _newColor, _oldColor){
  ...
  this.fillPixel(_x, _y, _newColor, _oldColor, stack);

  while(stack.length>0){
    toFill = stack.pop();
    this.fillPixel(toFill[0], toFill[1],_newColor, _oldColor, stack);
  }
}

PixelDrawer.prototype.fillPixel = function(_x,_y,_newColor, _oldColor, _stack){
  if(this.pixelNotColored(_x, _y, _newColor, _oldColor))
    this.colorPixel(_x,_y, _newColor);

  if(this.pixelNotColored(_x,   _y-1, _newColor, _oldColor)) _stack.push([_x,   _y-1]);
  if(this.pixelNotColored(_x+1, _y  , _newColor, _oldColor)) _stack.push([_x+1, _y  ]);
  if(this.pixelNotColored(_x,   _y+1, _newColor, _oldColor)) _stack.push([_x,   _y+1]);
  if(this.pixelNotColored(_x-1, _y  , _newColor, _oldColor)) _stack.push([_x-1, _y  ]);
}

PixelDrawer.prototype.pixelNotColored = function (_x, _y, _newColor, _oldColor) {
...
PixelDrawer.prototype.floodFillRecursive = function(_x, _y,_newColor, _oldColor)
{
  _newColor = this.checkIfParsedColor(_newColor);
